...
All colors are defined as CSS custom properties in `index.css` and mapped to Tailwind tokens via `@theme inline`. Components must use these tokens — no hardcoded hex values or raw Tailwind color classes like `zinc-*`.
...
- Dashboard: full-viewport layout — floating sidebar overlay on the left, center table - list of portfolio content entity data.
